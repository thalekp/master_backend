...
def test_volume_abnormality_maalarberget():
    ...
    imbalance = volume_abnormality('maalarberget', target_date)[0]
    assert imbalance<-1

def test_volume_abnormality_roan():
    target_date = date(2025, 1, 9)
    imbalance = volume_abnormality('roan', target_date)[0]
    assert imbalance<-1
...
def test_volume_abnormality_klevberget():
    ...
    imbalance = volume_abnormality('klevberget', target_date)[0]
    assert imbalance<-1
